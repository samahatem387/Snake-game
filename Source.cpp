...
#include "Snake.h" // Snake queue class
...
int main() {
	...
	cout << " Please enter the score to find its rank \n";
	cin >> element;
	...
	{
		mid = (first + last) / 2;
		if (score[mid] == element)
			found = true;

		else if (score[mid] > element)
			first = mid + 1;
	...
	if (found)
		cout << " This score is in the " << mid + 1 << " place" << endl;
...
void move_head() {   // Move head (according to its current direction) to its next node
	...
	if (frame[headypos][headxpos] == '#'
		|| frame[headypos][headxpos] == '#'
		|| frame[headypos][headxpos] == body_part)
		game_over = true;
